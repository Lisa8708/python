...
def gradient_penalty(discriminator, batch_x, fake_image):
    ...
    # grads: [b, h, w, c] => [b, -1]
    grads = tf.reshape(grads, [batch_x.shape[0], -1])
...
def d_loss_fn(generator, discriminator, batch_z, batch_x, training):
    ...
    loss = d_loss_real + d_loss_fake + gp
    return loss, gp
...
class Genarator(keras.Model):
    def __init__(self):
        ...
        # [b, 3, 3, 512] -> [b, 9, 9, 256] (batch_size, rows, cols, channels)
        # Conv2DTranspose(256, 3, 3, 'valid') -> filter=256, 卷积核kernel=3, 步长stride=3, padding=valid:No padding
...
# 3层CNN+FC，将[b, h, w, c] -> [b, ] 判断图片上的每个部分是否是真，以此判断图片真假
class Discriminator(keras.Model):
    def __init__(self):
        super(Discriminator, self).__init__()
        # new_rows = floor((rows-kernel)/stride) + 1, infer to:https://www.jianshu.com/p/2028177ca3e5
        # [b, 64, 64, 3] -> [b, 20, 20, 64]
        self.conv1 = layers.Conv2D(64, 5, 3, 'valid')
        # [b, 20, 20, 64] -> (b, 6, 6, 128)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        # train
        with tf.GradientTape() as tape:
            d_loss, gp = d_loss_fn(generator, discriminator, batch_z, batch_x, training)
        ...
        if epoch%100 == 0:
            print(f'epoch:{epoch}, d_loss={round(float(d_loss), 5)}, g_loss={round(float(g_loss), 5)}, gp={round(float(gp), 5)}')

            z = tf.random.normal([100, z_dim])
            fake_image = generator(z, training=False)
            img_path = os.path.join('images', f'wgan_fake_image_{epoch}.png')
